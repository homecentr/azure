resource "azuread_application" "whatsupdocker" {
  display_name     = "What's up Docker"
  sign_in_audience = "AzureADMyOrg"
  logo_image       = filebase64("../icons/whatsupdocker.png")

  web {
    redirect_uris = var.whatsupdocker_redirect_urls
  }
...
resource "azuread_service_principal" "whatsupdocker" {
  client_id                    = azuread_application.whatsupdocker.client_id
...
resource "azuread_service_principal_delegated_permission_grant" "whatsupdocker" {
  service_principal_object_id          = azuread_service_principal.whatsupdocker.object_id
  resource_service_principal_object_id = azuread_service_principal.msgraph.object_id
  claim_values                         = ["openid", "email", "profile", "offline_access"]
}

# Assign the app to the Administrators group
resource "azuread_app_role_assignment" "whatsupdocker_administrators" {
  ...
  resource_object_id  = azuread_service_principal.whatsupdocker.object_id
}
